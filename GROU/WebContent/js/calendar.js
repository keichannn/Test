...
(function() {
	...
	$$.prototype.start = function() {
		if (document.querySelector($$CALENDAR.prototype.option.table) === null) {
			return;
		}
	...
	$$CALENDAR.prototype.setYM = function(date) {
		...
		// this.option.date = {year:year.textContent , month:month.textContent ,
		// day:d.getDate()};
	...
	$$CALENDAR.prototype.getYM = function() {
		var y = $$CALENDAR.prototype.option.calendar
				.querySelector($$CALENDAR.prototype.option.year).textContent;
		var m = $$CALENDAR.prototype.option.calendar
				.querySelector($$CALENDAR.prototype.option.month).textContent;
		return {
			year : Number(y),
			month : Number(m)
		};
		...
		for (var row = 0; row < (endDate + startDay + (7 - endDay - 1)) / 7; row++) {
			...
			for (var col = 0; col < 7; col++) {
				if (row === 0 && col < startDay) {
					...
					// cls = (new Date(year, month-1, textDate) > new
					...
					// html += '<td class="'+cls+'">'+textDate+'</td>';
					// <% session.setAttribute("paidDay",'+textDate+') %>
	...
	$$CALENDAR.prototype.setEvent = function() {
		...
		$$LIB.prototype.setEvent(prev, "click",
				$$CALENDAR.prototype.prevCalendar);
		$$LIB.prototype.setEvent(next, "click",
				$$CALENDAR.prototype.nextCalendar);
		// day-click
		var days = this.option.calendar.querySelectorAll(".date");
		for (var i = 0; i < days.length; i++) {
			$$LIB.prototype.setEvent(days[i], "click",
					$$CALENDAR.prototype.dayClick);
		}
		var days = this.option.calendar.querySelectorAll(".current");
		for (var i = 0; i < days.length; i++) {
			$$LIB.prototype.setEvent(days[i], "click",
					$$CALENDAR.prototype.dayClick);
		}
	...
	$$CALENDAR.prototype.dayClick = function(e) {
		...
		var m = $$CALENDAR.prototype
				.numKeta($$CALENDAR.prototype.option.date.month);
		...
		location.href = 'YukyuSinseiServlet?paidDay='+(y + "/" + m + "/" + d);
	}
	// url.url+"?"+newQuery.join("&")
	...
	$$LIB.prototype.pathinfo = function(p) {
		var basename = "", dirname = [], filename = [], ext = "";
		var p2 = p.split("?");
		var urls = p2[0].split("/");
	...
	$$LIB.prototype.number_format = function(num) {
		num = num.toString();
		var tmpStr = "";
		while (num != (tmpStr = num.replace(/^([+-]?\d+)(\d\d\d)/, "$1,$2"))) {
			num = tmpStr;
		}
		return num;
	};
